...
- graph-aave-mcp: Aave V2/V3/V4 — 40 tools across 16 Graph subgraphs + Aave V4 API
  ...
  IMPORTANT — NEVER return `profitUSD` from an Aave V3 Messari-schema subgraph, and never vouch for it. Measured 2026-09-10 against the live gateway, `profitUSD` is wrong by roughly six orders of magnitude on BOTH deployments: Aave V3 Ethereum (JCNWRypm…) returns a median profit/amount ratio of -940,733x and Aave V3 Arbitrum (4xyasjQe…) -956,937x. Concretely, Arbitrum reported a $107,988,926 LOSS on a $112.85 liquidation and a $25,005,036 loss on a $26.58 one — 3 of 3 sampled rows implausible on each chain. This is a decimals defect in the subgraph mapping, not volatility, and the sign is always negative.
  `amountUSD` on the same rows is CORRECT and is what a liquidation question should be answered with. So: query and return amountUSD, liquidator, liquidatee, market and timestamp; omit profitUSD from `query_ready` entirely. If the caller explicitly asks for liquidator profit, give them the query WITHOUT that column and say plainly in `reason` that the subgraph's profitUSD is broken by ~1e6 and that profit must be derived from the seized-collateral and repaid-debt amounts instead. Handing over a field that claims a $108M loss on a $113 liquidation, unlabelled, is worse than not answering.
  This is the same class of defect as the Uniswap TVL rule above: the subgraph is the right one to use, and one of its columns cannot be trusted.
...
- "Aave liquidations above $50K" → subgraph-registry (Aave V3 subgraph, query LiquidationCall) — list graph-aave-mcp in alternatives. Return amountUSD, NEVER profitUSD (broken by ~1e6 on every Aave V3 deployment — see the graph-aave-mcp notes)
- "Which liquidators made the most profit on Aave?" → subgraph-registry (Aave V3 subgraph) BUT the query must not include profitUSD. Return liquidator + amountUSD ranked, and say in `reason` that the subgraph's profitUSD column is wrong by ~1e6 (it reports a $108M loss on a $113 liquidation), so liquidator profit cannot be read from it directly
...
def _fallback_route(request: str) -> dict:
    ...
    if svc == "token-api" and any(w in req for w in ["b20", "usdb", "scaledbalanceof", "scaled balance"]):
        ...
        reason = (
            ...
            "v3.21.0-pre1 + PR #578 polish merged 2026-06-18). Six endpoints "
            "under /v1/hyperliquid/outcomes/*: "
            "(1) /outcomes — universe with price + 24h metrics; "
            "(2) /outcomes/ohlc — per-leg OHLCV candles; "
            "(3) /outcomes/trades — taker fills feed, ?direction= CSV "
            "accepts BUY/SELL for trade-shaped buy/sell decomposition; "
            "(4) /outcomes/users — per-user-per-outcome P&L (omit ?user for "
            "leaderboard mode; ?sort_by accepts total_volume (default), "
            "transactions, realized_pnl); "
            "(5) /outcomes/users/positions — current share holdings; ?coin "
            "scopes to a single underlying (post-PR #578); "
            "(6) /outcomes/users/activity — COMPOSITION events only. "
            "?direction= here is a separate enum: SETTLEMENT, SPLIT_OUTCOME, "
            "MERGE_OUTCOME, MERGE_QUESTION, NEGATE_OUTCOME — passing BUY/SELL "
            "returns 400 (trade fills live on /outcomes/trades). Every "
            "response embeds OutcomeContext (outcome_id, question_id, "
            "status, settle_fraction) or OutcomeLegContext (adds coin, "
            "side_index, side_label). Heads-up: same release made "
            "/v1/hyperliquid/markets buy_volume_24h, sell_volume_24h, "
            "volume_24h, trades_24h taker-aggressor-only (~half previous "
            "magnitude) and removed buys/sells columns from /markets/ohlc, "
