...
async def fetch_user_aggregate(user: str) -> dict | None:
    """Full-history aggregate stats for a wallet from Pinax /users (FREE tier).

    Returns realized/unrealized/total PnL, volume, and transaction counts over
    the wallet's ENTIRE trading history — the authoritative PnL source. Unlike
    /users/positions (capped at 10 records and survivor-biased toward current
    winning holdings), this is not sampled, so it's what the skill score should
    be built on. Returns None if the wallet has no trading history.
    """
    rows = _data(await _pinax("/users", user=user))
...
def compute_scores(positions: list[dict], aggregate: dict | None = None) -> dict:
    """Wallet-level skill metrics.

    `positions` is the (≤10, survivor-biased) per-position sample used only for a
    consistency read. `aggregate` is the /users full-history row — when supplied
    it is the authoritative PnL and drives the score. Output fields are
    documented in the agent-card and the README.
    """
    ...
    # ── Authoritative PnL from the FULL-HISTORY /users aggregate ─────────────
    # The per-position figures above come from a ≤10-record, survivor-biased
    # sample — fine as a consistency read, wrong as the PnL of record. When the
    # caller supplies the /users aggregate (free tier), it covers the wallet's
    # ENTIRE history: use it for the verdict and report the true PnL.
    #   (2026-07-21: the sample said realized -$51k; the aggregate said
    #    -$32.4M — a 634x understatement, and the wallet was labeled "sharp 100".)
    if aggregate:
        a_real = float(aggregate.get("realized_pnl") or 0)
        a_unreal = float(aggregate.get("unrealized_pnl") or 0)
        a_total = float(aggregate.get("total_pnl") or 0)
        a_vol = float(aggregate.get("total_volume") or 0)
        a_txns = int(aggregate.get("transactions") or 0)
    else:
        a_real, a_unreal, a_total = realized_pnl, unrealized_pnl, total_pnl
        a_vol, a_txns = 0.0, transactions_total

    # With the aggregate, confidence legitimately reflects the full trade history
    # (millions of trades → high confidence is now correct, not a sampling bug).
    confidence = _clamp(math.log10(max(1, a_txns)) / 2.5, 0.0, 1.0)

    # Backbone: realized return on volume — does the wallet actually make money on
    # what it closes? Scaled so ~+12% realized ROI reads clearly "sharp" and
    # ~-12% clearly "retail". The open-position sharpe is a small secondary nudge
    # (±5 pts), never the driver. Realized dominates.
    realized_roi = a_real / a_vol if a_vol > 0 else 0.0
    skill_score = _clamp(
        50.0 + (realized_roi * 200.0 + _clamp(sharpe_like, -1.0, 1.0) * 5.0) * confidence,
        0.0, 100.0,
    )

    worst_position_pnl = min(
        (float(p.get("total_pnl") or 0) for p in closed_or_active), default=0.0,
    )

    enough = (a_txns >= 20) if aggregate else (sample_size >= 5)
    if not enough:
    ...
    if a_real < 0 <= a_total:
        caveats.append(
            f"Net PnL is positive (${a_total:,.0f}) only via UNREALIZED mark-to-market; "
            f"realized PnL is ${a_real:,.0f}. Paper gains on open positions can evaporate."
        )
    if a_vol > 0 and realized_roi <= -0.10:
        caveats.append(
            f"Realized return on volume is {realized_roi*100:.0f}% — this wallet loses "
            "money on what it actually closes."
        )
    if win_rate >= 0.99 and sample_size < 30:
        caveats.append(
            "win_rate is over the sampled open positions only (survivor-biased) and does "
            "NOT drive the score — it is not a full-history record."
        )
    if not aggregate:
        caveats.append(
    ...
    return {
        ...
        "realized_roi_pct": round(realized_roi * 100, 2),
        "sharpe_like": round(sharpe_like, 3),
        "win_rate": round(win_rate, 3),
        "realized_pnl_usdc": round(a_real, 2),
        "unrealized_pnl_usdc": round(a_unreal, 2),
        "total_pnl_usdc": round(a_total, 2),
        "total_volume_usdc": round(a_vol, 2),
        "sample_size_trades": a_txns,
        "sample_size_markets": sample_size,
        "positions_scored": sample_size,
        "sample_capped": sample_capped,
        "pnl_source": "users-aggregate-full-history" if aggregate else "position-sample-only",
...
async def score_wallet(wallet: str) -> dict:
    """Convenience: fetch the full-history aggregate + position sample, then score.

    Both fetches run concurrently; either failing degrades that input rather than
    the whole call (missing aggregate → sample-only score with a caveat)."""
    import asyncio
    positions, aggregate = await asyncio.gather(
        fetch_user_positions(wallet),
        fetch_user_aggregate(wallet),
        return_exceptions=True,
    )
    if isinstance(positions, Exception):
        positions = []
    if isinstance(aggregate, Exception):
        aggregate = None
    return compute_scores(positions, aggregate)
