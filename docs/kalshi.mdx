...
| `POST /kalshi-polymarket/spread` | $0.05 | Cross-source spread between Kalshi and Polymarket on the same topic, with arbitrage direction. **JOIN that passthrough APIs (incl. Pinax Token API) structurally can't return** |
...
Graph Advocate does it once, exposes the answer. The cross-source `/spread` endpoint in particular is the **defensible play**: it's still useful even if [Pinax adds Kalshi to Token API](https://docs.pinax.network/), because passthrough APIs return one source at a time and can't JOIN.
...
```json
{
  ...
  "agent_note": "Spread > 200bps in either direction is a candidate arbitrage; verify the two markets actually resolve on the same condition before sizing. Cross-source data Pinax can't return in one call.",
  "sources": {
    "kalshi": "https://api.elections.kalshi.com/trade-api/v2",
    "polymarket_via": "pinax-token-api"
  }
}
```
...
These three Kalshi endpoints are **derived signals** on top of a *fully public* upstream — different model than the polymarket/hyperliquid trader-intelligence endpoints which require Pinax Token API auth. Kalshi's API is free for anyone to read; Graph Advocate's value here is in the math:
...
[`kalshi.py`](https://github.com/PaulieB14/graph-advocate/blob/main/kalshi.py) — the three derivation functions and how they call Kalshi's public REST + Pinax's Polymarket Token API.
